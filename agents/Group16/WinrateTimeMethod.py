...
def allocate_thinking_time(
    ...
    C = C * C_multiplier if extra_time else C
...
class MCTSNode:
    ...
    def compute_winrate(self) -> float:
        if self.visits == 0:
            return 0.5
        avg_reward = self.value / self.visits
        return (avg_reward + 1) / 2
...
def run_mcts_loop(root: MCTSNode, my_colour: Colour, max_time_seconds: float, max_iterations: int, it: int, pool, workers: int):
...
def mcts_search(
    ...
    root_winrate_threshold: float = 0.45,
) -> Move:
    """
    ...
    first_loop_start_time = time.perf_counter()
    ...
    with Pool(processes=workers) as pool:
        run_mcts_loop(root, my_colour, max_time_seconds, max_iterations, it, pool, workers)

        # If winrate is too low, carry on searching with extra time
        winrate = root.compute_winrate()
        print(f"After MCTS:")
        print(f"   Root Value: {root.value}")
        print(f"   Root Visits: {root.visits}")
        print(f"   Winrate: {root.value/root.visits}")
        print(f"   Converted Winrate: {winrate}")

        if winrate < root_winrate_threshold:
            # Get extra time and re-run loop
            first_loop_end_time = time.perf_counter()
            agent.time_spent += first_loop_end_time - first_loop_start_time
            extra_time = allocate_thinking_time(agent.time_spent, turn, extra_time=True)
            print(f"Winrate too low, running with extra time: {extra_time}s")
            run_mcts_loop(root, my_colour, extra_time, max_iterations, it, pool, workers)

            winrate = root.compute_winrate()
            print(f"After Second MCTS Loop:")
            print(f"   Root Value: {root.value}")
            print(f"   Root Visits: {root.visits}")
            print(f"   Winrate: {root.value/root.visits}")
            print(f"   Converted Winrate: {winrate}")
...
class WinrateTimeMethodAgent(AgentBase):
    _board_size: int = 11
    ...
    def make_move(self, turn: int, board: Board, opp_move: Move | None) -> Move:
        ...
        searching_time = allocate_thinking_time(self.time_spent, turn)
        if len(move_set) == 1:
